...
func (cluster *Cluster) Relay(peer string, c scheme.ServerConnection, args [][]byte) scheme.Reply {
...
func defaultFunc(cluster *Cluster, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	key := string(args[1])
	peer := cluster.peerPicker.Get(key)
	return cluster.Relay(peer, c, args)
}

func Ping(cluster *Cluster, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	if len(args) == 1 {
...
func Rename(cluster *Cluster, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	if len(args) != 3 {
...
func RenameNx(cluster *Cluster, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	if len(args) != 3 {
...
func MSetNX(cluster *Cluster, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	argCount := len(args) - 1
...
func Del(cluster *Cluster, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	if len(args) < 2 {
...
func MGet(cluster *Cluster, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	if len(args) < 2 {
		return reply.MakeErrReply("ERR wrong number of arguments for 'del' command")
	}
	keys := make([]string, len(args)-1)
...
func MSet(cluster *Cluster, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	argCount := len(args) - 1
	if argCount%2 != 0 || argCount < 1 {
		return reply.MakeErrReply("ERR wrong number of arguments for 'mset' command")
	}
