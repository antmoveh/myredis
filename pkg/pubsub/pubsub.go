 ...
 */
func subscribe0(hub *Hub, channel string, client scheme.ServerConnection) bool {
 ...
 */
func unsubscribe0(hub *Hub, channel string, client scheme.ServerConnection) bool {
...
func Subscribe(hub *Hub, c scheme.ServerConnection, args [][]byte) scheme.Reply {
...
func UnsubscribeAll(hub *Hub, c scheme.ServerConnection) {
...
func UnSubscribe(db *Hub, c scheme.ServerConnection, args [][]byte) scheme.Reply {
	var channels []string
	if len(args) > 0 {
		channels = make([]string, len(args))
		for i, b := range args {
...
func Publish(hub *Hub, args [][]byte) scheme.Reply {
	if len(args) != 2 {
	...
	subscribers.ForEach(func(i int, c interface{}) bool {
		client, _ := c.(scheme.ServerConnection)
