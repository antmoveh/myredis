...
func (db *DB) addAof(args *reply.MultiBulkReply) {
	...
	if configuration.AppendOnly && db.aofChan != nil {
		db.aofChan <- args
	}
}
...
func (db *DB) loadAof(maxBytes int) {
	...
	for {
		...
		if fixedLen == 0 {
			...
			if len(msg) == 0 {
				logrus.Warn("invalid format: line should end with \\r\\n")
				return
			}
			readBytes += len(msg)
		} else {
			...
			if len(msg) == 0 {
				logrus.Warn("invalid multibulk length")
				return
			}
			fixedLen = 0
			readBytes += n
		...
		if !processing {
			...
			if msg[0] == '*' {
				...
				logrus.Warn("msg should start with '*'")
				return
			}
		} else {
...
func (db *DB) startRewrite() (*os.File, int64, error) {
	...
	if err != nil {
		logrus.Warn("tmp file create failed")
		return nil, 0, err
	}
