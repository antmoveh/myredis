...
type CmdFunc func(db *DB, args [][]byte) scheme.Reply
...
func MakeDB() *DB {
	...
	// aof
	if configuration.AppendOnly {
		db.aofFilename = configuration.AppendFileName
		db.loadAof(0)
...
func (db *DB) Exec(c scheme.ServerConnection, args [][]byte) (result scheme.Reply) {
	defer func() {
...
func (db *DB) AfterClientClose(c scheme.ServerConnection) {
