...
import (
	...
	SortedSet "myredis/pkg/types/data/sortedset"
...
func ZAdd(db *DB, args [][]byte) scheme.Reply {
...
func ZScore(db *DB, args [][]byte) scheme.Reply {
	// parse args
	if len(args) != 2 {
...
func ZRank(db *DB, args [][]byte) scheme.Reply {
	// parse args
	if len(args) != 2 {
...
func ZRevRank(db *DB, args [][]byte) scheme.Reply {
	// parse args
	if len(args) != 2 {
...
func ZCard(db *DB, args [][]byte) scheme.Reply {
	// parse args
	if len(args) != 1 {
...
func ZRange(db *DB, args [][]byte) scheme.Reply {
	// parse args
...
func ZRevRange(db *DB, args [][]byte) scheme.Reply {
	// parse args
	if len(args) != 3 && len(args) != 4 {
...
func range0(db *DB, key string, start int64, stop int64, withScores bool, desc bool) scheme.Reply {
	// lock key
	db.Locker.RLock(key)
...
func ZCount(db *DB, args [][]byte) scheme.Reply {
	if len(args) != 3 {
 ...
 */
func rangeByScore0(db *DB, key string, min *SortedSet.ScoreBorder, max *SortedSet.ScoreBorder, offset int64, limit int64, withScores bool, desc bool) scheme.Reply {
	// lock key
	db.Locker.RLock(key)
	defer db.Locker.RUnLock(key)
...
func ZRangeByScore(db *DB, args [][]byte) scheme.Reply {
	if len(args) < 3 {
...
func ZRevRangeByScore(db *DB, args [][]byte) scheme.Reply {
	if len(args) < 3 {
		return reply.MakeErrReply("ERR wrong number of arguments for 'zrangebyscore' command")
	}
	key := string(args[0])
...
func ZRemRangeByScore(db *DB, args [][]byte) scheme.Reply {
	if len(args) != 3 {
...
func ZRemRangeByRank(db *DB, args [][]byte) scheme.Reply {
	if len(args) != 3 {
...
func ZRem(db *DB, args [][]byte) scheme.Reply {
	// parse args
	if len(args) < 2 {
...
func ZIncrBy(db *DB, args [][]byte) scheme.Reply {
	if len(args) != 3 {
